...
std::vector<float> sequential_values(int size, float scale = 1.0f, float offset = 0.0f) {
...
int output_elements(int batch, int channels_out, int height, int width, int kernel_h, int kernel_w) {
...
void expect_im2col_matches_naive(const std::vector<float>& input,
    ...
    kernels::conv2d_naive(input.data(), kernel.data(), expected.data(), batch, channels_in,
                          height, width, channels_out, kernel_h, kernel_w);
    kernels::conv2d_im2col(input.data(), kernel.data(), actual.data(), batch, channels_in,
                           height, width, channels_out, kernel_h, kernel_w);
...
void conv2d_manual_3x3_2x2() {
    ...
    kernels::conv2d_naive(input.data(), kernel.data(), output.data(), 1, 1, 3, 3, 1, 2, 2);

    test::expect_vector_near(output, {-4.0f, -4.0f, -4.0f, -4.0f}, kEps,
                             "manual 3x3 2x2 conv");
    expect_im2col_matches_naive(input, kernel, 1, 1, 3, 3, 1, 2, 2,
                                "manual 3x3 2x2 im2col conv");
}

void conv2d_single_channel_5x5_3x3() {
    const std::vector<float> input = sequential_values(1 * 1 * 5 * 5, 0.25f, -2.0f);
    const std::vector<float> kernel = sequential_values(1 * 1 * 3 * 3, -0.125f, 1.0f);

    expect_im2col_matches_naive(input, kernel, 1, 1, 5, 5, 1, 3, 3,
                                "single-channel 5x5 3x3");
}

void conv2d_multi_channel_multi_output() {
    const std::vector<float> input = sequential_values(1 * 3 * 8 * 8, 0.01f, -1.0f);
    const std::vector<float> kernel = sequential_values(4 * 3 * 3 * 3, -0.015f, 0.75f);

    expect_im2col_matches_naive(input, kernel, 1, 3, 8, 8, 4, 3, 3,
                                "N=1 C_in=3 C_out=4 8x8 3x3");
}

void conv2d_multi_batch_multi_channel() {
    const std::vector<float> input = sequential_values(2 * 3 * 8 * 8, 0.02f, -3.0f);
    const std::vector<float> kernel = sequential_values(5 * 3 * 3 * 3, 0.01f, -0.5f);

    expect_im2col_matches_naive(input, kernel, 2, 3, 8, 8, 5, 3, 3,
                                "N=2 C_in=3 C_out=5 8x8 3x3");
}

void conv2d_kernel_1x1() {
    const std::vector<float> input = sequential_values(1 * 3 * 4 * 4, 0.1f, -2.0f);
    const std::vector<float> kernel = sequential_values(2 * 3 * 1 * 1, 0.2f, -0.3f);
...
void reshape_kernel_small_direct() {
    ...
    kernels::reshape_kernel_for_im2col(kernel.data(), kernel_matrix.data(), 2, 1, 2, 2);
...
std::vector<test::TestCase> conv2d_tests() {
    return {
        {"conv2d_naive computes manual 3x3 input with 2x2 kernel", conv2d_manual_3x3_2x2},
