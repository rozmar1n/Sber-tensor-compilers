...
std::vector<float> sequential_matrix(int size, float scale = 1.0f, float offset = 0.0f) {
...
void expect_all_variants_match_naive(const std::vector<float>& A,
    ...
    expect_variant_matches_naive(kernels::matmul_cache_friendly,
                                 std::string(case_name) + " cache-friendly", A, B, M, K, N);
    expect_variant_matches_naive(
        [](const float* A_ptr, const float* B_ptr, float* C_ptr, int m, int k, int n) {
            kernels::matmul_tiled(A_ptr, B_ptr, C_ptr, m, k, n, kTileSize);
        },
        std::string(case_name) + " tiled", A, B, M, K, N);
    expect_variant_matches_naive(kernels::matmul_vectorized,
                                 std::string(case_name) + " vectorized", A, B, M, K, N);
}
...
void matmul_non_avx_tail_7x10_10x9() {
    ...
    expect_all_variants_match_naive(A, B, 7, 10, 9, "non-AVX-tail 7x10 by 10x9");
}
...
void matmul_non_tile_aligned_31x17_17x23() {
    ...
    expect_all_variants_match_naive(A, B, 31, 17, 23,
                                    "non-tile-aligned 31x17 by 17x23");
}
...
void matmul_deterministic_random_data() {
    ...
    expect_all_variants_match_naive(A, B, 13, 11, 7, "deterministic random data");
}
...
std::vector<test::TestCase> matmul_tests() {
    return {
        ...
        {"optimized matmul computes rectangular 3x5 by 5x4", matmul_rectangular_3x5_5x4},
        {"optimized matmul handles non-AVX tail", matmul_non_avx_tail_7x10_10x9},
        ...
        {"optimized matmul handles deterministic random data", matmul_deterministic_random_data},
    };
}
